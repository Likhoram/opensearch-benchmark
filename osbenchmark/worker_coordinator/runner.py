...
"""
Runner module for OpenSearch Benchmark.

Contains the OpenSearch-specific runner implementations along with the runner
base classes (Runner, Delegator, MultiClientRunner, etc.), utility helpers,
and the runner registry (register_runner, runner_for, register_default_runners).

This module lives at its historical 2.1 path so the workloads repository
(opensearch-project/opensearch-benchmark-workloads) can import OpenSearch
runner classes without changes:
